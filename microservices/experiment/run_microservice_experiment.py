...
def _restart_service(project_root: Path, service_name: str) -> None:
...
async def main() -> None:
    parser = argparse.ArgumentParser(description="Run microservice restart loss experiment.")
    parser.add_argument("--duration", type=float, default=45.0, help="Total test duration in seconds.")
    parser.add_argument(
        "--restart-at",
        type=float,
        default=15.0,
        help="Seconds from test start to restart the target service.",
    )
    parser.add_argument(
        "--target-service",
        type=str,
        default="purchase_service",
        help="Service to restart during the test.",
    )
    ...
    endpoints = [
        Endpoint("purchase", "POST", "http://localhost:8001/purchase", 0.4),
        Endpoint("download", "POST", "http://localhost:8002/download", 0.4),
        Endpoint("analytics", "GET", "http://localhost:8003/analytics", 0.2),
    ]
    ...
    print("Running microservice experiment...")
    ...
    async def disruption() -> None:
        print(f"[{time.strftime('%H:%M:%S')}] Restarting {args.target_service} ...")
        loop = asyncio.get_running_loop()
        await loop.run_in_executor(None, _restart_service, project_root, args.target_service)
    ...
    report = _format_report("microservice_restart", result)
